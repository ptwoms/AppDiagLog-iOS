    ...
    var body: some View {
        List {
            Section("Domain Event Scenarios") {
                Text("Tap any scenario to emit a realistic event. UIKit owns the tab bar and navigation; SwiftUI renders this content.")
                    .font(.footnote)
                    .foregroundColor(.secondary)

                eventButton("User Login", icon: "person.fill.checkmark", tint: .blue) {
                    AppDiagLog.info("user_login", ["method": "email", "duration_ms": "120", "source": "ios_uikit_events_lab"])
                ...
                eventButton("Feature Flag Check", icon: "flag", tint: .teal) {
                    AppDiagLog.debug("feature_flag_check", ["flag": "new_checkout_flow", "result": "enabled", "source": "ios_uikit_events_lab"])
            ...
            Section("Rate Limiter Demo") {
                Text("Fire 200 events immediately. The SDK allows max \(SampleConfiguration.maxEventsPerSecond) events/s — events beyond the cap are silently dropped.")
                    .font(.footnote)
                    .foregroundColor(.secondary)
